<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800">Editar reserva</h2></x-slot>
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <x-flash-messages />
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('reservas.update', $reserva) }}" class="space-y-4">
                    @csrf @method('PUT')
                    <div>
                        ...
                        <select id="id_cliente" name="id_cliente" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm" required>
                            @foreach ($clientes as $cliente)
                                <option value="{{ $cliente->id_cliente }}" @selected(old('id_cliente', $reserva->id_cliente) == $cliente->id_cliente)>{{ $cliente->nombres }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select id="id_disponibilidad" name="id_disponibilidad" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm" required>
                            @foreach ($disponibilidades as $item)
                                <option value="{{ $item->id_disponibilidad }}" @selected(old('id_disponibilidad', $reserva->id_disponibilidad) == $item->id_disponibilidad)>
                                    {{ $item->paquete->nombre_paquete }} — {{ $item->fecha->format('d/m/Y') }} ({{ $item->cupos_disponibles }} cupos)
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select id="estado" name="estado" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm" required>
                            @foreach (['pendiente', 'confirmada', 'cancelada', 'completada'] as $estado)
                                <option value="{{ $estado }}" @selected(old('estado', $reserva->estado) === $estado)>{{ ucfirst($estado) }}</option>
                            @endforeach
                        </select>
                    </div>
